...
var (
	...
	expSize      int       = 250
	cipher       []Struct1 = make([]Struct1, expSize)
...
func post(instance *TestABI, auth *bind.TransactOpts, conn *ethclient.Client) {
	auth.Nonce = nil
	tx, err := instance.Store(auth, cipher[0:150])
	if err != nil {
		fmt.Println(err)
	}
	...
	tx, err = instance.Store(auth, cipher[150:expSize])
	if err != nil {
		fmt.Println(err)
	}
	ctx = context.Background()
...
func main() {
	...
	scAddress = "0x5b49DFC13a6DaFd31af00228A86d924dB6c3C491"
