...
// func enc(w string, id uint, index uint): Encrypt the keyword w and id and add it into the cipher set
func enc(w string, id int, index uint) float64 {
	...
	elapsed := float64(time.Since(t).Microseconds()) / 1000
	return elapsed
}
...
func main() {
	...
	for expTime := 0; expTime < 10; expTime++ {
		for i := 0; i < expSize; i++ {
			exp[expTime] += enc(Vocab[i].key, Vocab[i].id, uint(i))
		}
	}
	fmt.Println(exp)
}
